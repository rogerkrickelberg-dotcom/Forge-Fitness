...
const { Activity, Dumbbell, Zap, Bike, Footprints, TrendingUp, Camera, Plus, Trash2, X, Save, Scale, Heart, Flame, Gauge, ArrowLeft } = window.LucideReact;
...
const STORAGE_PREFIX = "forge_fitness_";
...
// ---------- Strava import ----------
function parseStravaPaste(raw) {
  let data;
  try { data = JSON.parse(raw); } catch { throw new Error("That doesn't look like valid JSON — check you copied the whole block."); }
  if (!data.date || (data.distance == null && data.duration == null)) {
    throw new Error("Missing required fields — need at least date and distance or duration.");
  }
  const distance = data.distance != null ? Math.round(data.distance * 100) / 100 : "";
  const duration = data.duration != null ? Math.round(data.duration * 10) / 10 : "";
  let avgPace = null;
  ...
  return {
    id: uid(), kind: "cardio",
    type: ["running", "bike", "elliptical"].includes(data.type) ? data.type : "running",
    date: data.date, distance: distance || 0, duration: duration || 0,
    avgHR: data.avgHR != null ? Math.round(data.avgHR) : null,
    calories: data.calories != null ? Math.round(data.calories) : null,
    avgPace, source: "strava",
    zones: Array.isArray(data.zones) ? data.zones : [],
    splits: Array.isArray(data.splits) ? data.splits.map((sp) => ({ id: uid(), label: String(sp.label ?? ""), pace: sp.pace || "", hr: sp.hr != null ? String(sp.hr) : "" })) : [],
    notes: data.notes || "",
  };
}

function StravaImportBox({ onParsed, onCancel }) {
  const [raw, setRaw] = useState("");
  const [error, setError] = useState("");

  const handleImport = () => {
    try {
      const parsed = parseStravaPaste(raw);
      setError("");
      onParsed(parsed);
    } catch (e) {
      setError(e.message);
  ...
  return (
    <Card style={{ display: "flex", flexDirection: "column", gap: 10, border: `1px solid ${TERTIARY_CONTAINER}55` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 16, textTransform: "uppercase", color: TERTIARY }}>Paste from Strava</div>
        <button onClick={onCancel} style={{ background: "none", border: "none", color: ON_SURFACE_VARIANT, cursor: "pointer" }}><X size={16} /></button>
      </div>
      <p style={{ fontSize: 12, color: ON_SURFACE_VARIANT, margin: 0, lineHeight: 1.5 }}>
        Ask Claude in chat to pull an activity from Strava — it'll give you a JSON block. Paste it below to pre-fill the cardio form.
      </p>
      <textarea
        value={raw}
        onChange={(e) => setRaw(e.target.value)}
        placeholder='{"type":"running","date":"2026-08-12","distance":4.04,"duration":23.3,...}'
        rows={5}
        style={{ ...inputStyle, fontFamily: FONT_MONO, fontSize: 11.5, resize: "vertical" }}
      />
      {error && <div style={{ fontSize: 12, color: ERROR }}>{error}</div>}
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <Btn variant="ghost" onClick={onCancel}>Cancel</Btn>
        <Btn variant="cyan" onClick={handleImport} style={{ background: TERTIARY_CONTAINER, color: "#fff" }}>Parse and review</Btn>
      </div>
    </Card>
  );
}
...
// ---------- Dashboard ----------
function Dashboard({ cardio, strength, bodyLogs, onGo, onImport }) {
  ...
  const [syncMode, setSyncMode] = useState(null); // null | "export" | "import"
  const [importText, setImportText] = useState("");
  const [importError, setImportError] = useState("");
  const [importSuccess, setImportSuccess] = useState("");
  const exportBlob = useMemo(
    () => JSON.stringify({ cardio, strength, bodyLogs, exportedAt: new Date().toISOString() }),
    [cardio, strength, bodyLogs, syncMode]
  );

  const handleImport = () => {
    try {
      const parsed = JSON.parse(importText);
      if (!parsed || (!parsed.cardio && !parsed.strength && !parsed.bodyLogs)) {
        throw new Error("That doesn't look like a Forge export — check you copied the whole block.");
      }
      onImport(parsed);
      setImportSuccess(
        `Imported ${(parsed.cardio || []).length} cardio, ${(parsed.strength || []).length} strength, ${(parsed.bodyLogs || []).length} body entries.`
      );
      setImportError("");
      setImportText("");
    } catch (e) {
      setImportError(e.message || "Couldn't parse that — check you copied the whole block.");
      setImportSuccess("");
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div>
        <Label>Sync between browser tab and home-screen app</Label>
        <div style={{ fontSize: 11.5, color: ON_SURFACE_VARIANT, marginBottom: 8, lineHeight: 1.5 }}>
          Each browser context (regular tab vs. home-screen icon) stores data separately. Export
          from one, import into the other, to bring entries across.
        </div>
        {!syncMode && (
          <div style={{ display: "flex", gap: 8 }}>
            <Btn variant="ghost" onClick={() => { setSyncMode("export"); setImportSuccess(""); setImportError(""); }} style={{ flex: 1, justifyContent: "center" }}>
              Export
            </Btn>
            <Btn variant="ghost" onClick={() => { setSyncMode("import"); setImportSuccess(""); setImportError(""); }} style={{ flex: 1, justifyContent: "center" }}>
              Import
            </Btn>
          </div>
        )}
        {syncMode === "export" && (
          <Card style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ fontSize: 12, color: ON_SURFACE_VARIANT, lineHeight: 1.5 }}>
              Tap the box below, select all, and copy. Paste it into "Import" on the other version of the app.
            </div>
            <textarea
              readOnly
              value={exportBlob}
              ...
              rows={6}
              style={{ ...inputStyle, fontFamily: FONT_MONO, fontSize: 10.5, resize: "vertical" }}
            />
            <Btn variant="ghost" onClick={() => setSyncMode(null)} style={{ alignSelf: "flex-end" }}>Close</Btn>
          </Card>
        )}
        {syncMode === "import" && (
          <Card style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ fontSize: 12, color: ON_SURFACE_VARIANT, lineHeight: 1.5 }}>
              Paste an export blob from the other version of the app. Existing entries are kept —
              this only adds what's missing.
            </div>
            <textarea
              value={importText}
              onChange={(e) => setImportText(e.target.value)}
              placeholder='{"cardio":[...],"strength":[...],"bodyLogs":[...]}'
              rows={6}
              style={{ ...inputStyle, fontFamily: FONT_MONO, fontSize: 10.5, resize: "vertical" }}
            />
            {importError && <div style={{ fontSize: 12, color: ERROR }}>{importError}</div>}
            {importSuccess && <div style={{ fontSize: 12, color: SECONDARY }}>{importSuccess}</div>}
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <Btn variant="ghost" onClick={() => setSyncMode(null)}>Close</Btn>
              <Btn variant="secondary" onClick={handleImport}>Import</Btn>
            </div>
          </Card>
        )}
      </div>
...
function App() {
  ...
  const addCardio = (s) => { setCardio((prev) => [s, ...prev]); setShowCardioForm(false); setImportedDraft(null); };
  ...
  // Merge an exported backup (from another browser/storage context) into current data.
  // De-duplicates by id so importing the same backup twice is harmless.
  const importBackup = (parsed) => {
    ...
    setCardio((prev) => mergeById(prev, parsed.cardio));
    setStrength((prev) => mergeById(prev, parsed.strength));
    setBodyLogs((prev) => mergeById(prev, parsed.bodyLogs));
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "20px 20px 0" }}>
        {tab === "dashboard" && <Dashboard cardio={cardio} strength={strength} bodyLogs={bodyLogs} onGo={setTab} onImport={importBackup} />}

        {tab === "cardio" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {!showCardioForm && !showStravaImport && (
              <div style={{ display: "flex", gap: 8 }}>
                <Btn variant="secondary" onClick={() => setShowCardioForm(true)} style={{ justifyContent: "center", flex: 1 }}><Plus size={15} /> Log manually</Btn>
                <Btn variant="ghost" onClick={() => setShowStravaImport(true)} style={{ justifyContent: "center", flex: 1, borderColor: `${TERTIARY_CONTAINER}55`, color: TERTIARY }}><Activity size={15} /> Paste from Strava</Btn>
              </div>
            )}
            {showStravaImport && (
              <StravaImportBox
                onParsed={(draft) => { setImportedDraft(draft); setShowStravaImport(false); setShowCardioForm(true); }}
                onCancel={() => setShowStravaImport(false)}
              />
            )}
            {showCardioForm && (
              <CardioForm
                ...
                onSave={addCardio}
                onCancel={() => { setShowCardioForm(false); setImportedDraft(null); }}
              />
            )}
            {cardioSorted.map((s) => <CardioSessionCard key={s.id} s={s} onDelete={deleteCardio} />)}
            {cardioSorted.length === 0 && !showCardioForm && !showStravaImport && <div style={{ color: OUTLINE, fontSize: 12.5, textAlign: "center", padding: 30, fontFamily: FONT_BODY }}>No cardio sessions logged yet.</div>}
          </div>
        )}
